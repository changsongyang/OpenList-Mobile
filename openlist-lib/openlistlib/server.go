...
// Shutdown 优雅关闭服务器，包括调用cmd.Release()
func Shutdown(timeout int64) (err error) {
	...
	if conf.Conf.Scheme.HttpPort != -1 && httpSrv != nil {
		...
		if err != nil {
			utils.Log.Error("HTTP server shutdown error: ", err)
		} else {
	...
	if conf.Conf.Scheme.HttpsPort != -1 && httpsSrv != nil {
		...
		if err != nil {
			utils.Log.Error("HTTPS server shutdown error: ", err)
		} else {
	...
	if conf.Conf.Scheme.UnixFile != "" && unixSrv != nil {
		...
		if err != nil {
			utils.Log.Error("Unix server shutdown error: ", err)
		} else {
